...
const google = (key: string, service: string, api: string, documentationURL: string): OAuthProviderSetup => ({
  ...
  steps: [
    ...
    ["เพิ่ม Callback URL ด้านล่างในช่อง Authorized redirect URIs แล้วคัดลอก Client ID และ Client Secret มาใส่ที่นี่", "Add the Callback URL below to Authorized redirect URIs, then copy the Client ID and Client Secret here."],
    [`เปิดใช้ ${api} ในโครงการที่จะใช้งาน และกำหนดบทบาท MCP Tool User พร้อมสิทธิ์ ${service} ให้สมาชิกเท่าที่งานต้องใช้`, `Enable ${api} in the target project. Grant members MCP Tool User and only the ${service} permissions their work needs.`],
  ],
});
...
const providers: Record<string, OAuthProviderSetup> = {
  slack: {
    ...
    steps: [
      ["สร้างแอป ORCA ใน Slack Workspace ที่จะใช้ ปุ่มสร้างแอปกำหนด Callback URL และสิทธิ์อ่านช่องสาธารณะไว้ให้แล้ว", "Create ORCA in the Slack workspace you will use. The create-app button includes the callback and public-channel read scopes."],
      ["เลือก Install App เพื่อติดตั้งใน Workspace แล้วนำ Client ID และ Client Secret จากหน้า Basic Information มาใส่ด้านล่าง", "Use Install App to install into your workspace, then enter the Client ID and Client Secret from Basic Information below."],
      ["แอปภายในใช้ได้กับ Workspace ขององค์กรเท่านั้น การเปิดให้องค์กรอื่นใช้งานต้องเป็นไปตามข้อกำหนดการเผยแพร่แอปของ Slack", "Use an internal app for your own organization's workspace. Distribution to other organizations must meet Slack's app publishing requirements."],
    ],
  },
  asana: {
    ...
    steps: [
      ...
      ["เพิ่ม Callback URL ด้านล่างในส่วน OAuth แล้วเลือก Workspace ที่อนุญาตในส่วน Manage distribution", "Add the Callback URL below in OAuth, then select the permitted workspaces in Manage distribution."],
  ...
  box: {
    ...
    steps: [
      ...
      ["เลือก Access Scopes ที่ต้องใช้ แล้วนำ Client ID และ Client Secret มาใส่ที่นี่ สิทธิ์ docgen.readwrite ต้องใช้แพ็กเกจ Enterprise Advanced", "Select the required Access Scopes, then enter the Client ID and Client Secret here. docgen.readwrite requires Enterprise Advanced."],
    ],
  },
  docusign: {
    ...
    steps: [
      ...
      ["ใช้คีย์ให้ตรงกับปลายทาง บัญชีนักพัฒนาใช้ mcp-d.docusign.com ส่วนรายการนี้ใช้ mcp.docusign.com ซึ่งต้องใช้แอปที่เปิดใช้งานจริง (production)", "Match credentials to the server: developer accounts use mcp-d.docusign.com; this mcp.docusign.com entry requires a production-enabled integration."],
    ],
  },
  hubspot: {
    ...
    steps: [
      ...
      ["ใส่ Callback URL ด้านล่างในช่อง Redirect URL แล้วสร้างตัวเชื่อม", "Enter the Callback URL below as Redirect URL and create the connector."],
      ["นำ Client ID และ Client secret จากหน้ารายละเอียดมาใส่ที่นี่ สมาชิกจะเลือกบัญชี HubSpot และอนุญาตสิทธิ์เมื่อเชื่อมบัญชี", "Enter the Client ID and Client secret from its details here. Members choose their HubSpot account and grant permissions when connecting."],
    ],
  },
  salesforce: {
    ...
    steps: [
      ...
      ["เปิดใช้ OAuth ใส่ Callback URL ด้านล่าง เลือกสิทธิ์ mcp_api และ refresh_token แล้วตั้งค่า JWT และความปลอดภัยตามคู่มือ Hosted MCP", "Enable OAuth, add the Callback URL below, select mcp_api and refresh_token, and follow the Hosted MCP guide for JWT and security settings."],
      ["นำ Consumer Key และ Consumer Secret มาใส่ที่นี่ แล้วระบุ URL ของระบบขององค์กรในขั้นตอนเชื่อมบัญชี แอปใหม่อาจใช้เวลาเปิดใช้งานถึง 30 นาที", "Enter the Consumer Key and Consumer Secret here, then provide the org's server during account setup. A new app can take up to 30 minutes to activate."],
    ],
  },
  snowflake: {
    ...
    steps: [
      ...
      ["ใช้ Client ID และ Client Secret ของ integration นี้ จำกัดบทบาท (role) ให้ตรงกับงาน และให้สิทธิ์เข้าถึง MCP server และเครื่องมือที่จะใช้", "Use that integration's Client ID and Client Secret. Limit the role to the intended work and grant access to the MCP server and its tools."],
  ...
  "github-enterprise": {
    ...
    steps: [
      ["รายการนี้ใช้กับ GitHub Enterprise Cloud ที่กำหนดพื้นที่จัดเก็บข้อมูล (data residency) ให้ผู้ดูแลยืนยันโดเมนขององค์กรและเปิดใช้งาน MCP ตามคู่มือ", "This entry is for GitHub Enterprise Cloud with data residency. Have your admin confirm the enterprise domain and enable MCP using the guide."],
  ...
  zoom: {
    ...
    steps: [
      ["เข้าสู่ Zoom Marketplace ด้วยบัญชีผู้ดูแลหรือนักพัฒนา เลือก Develop → Build app → General app แล้วตั้งชื่อ ORCA", "Sign in to Zoom Marketplace as an admin or developer. Choose Develop → Build app → General app and name it ORCA."],
      ["ใส่ Callback URL ด้านล่าง และเลือกสิทธิ์ (Scopes) ตามเครื่องมือและผลิตภัณฑ์ Zoom ที่จะใช้", "Add the Callback URL below and select scopes for the Zoom tools and products you will use."],
      ["นำ Client ID และ Client Secret มาใส่ที่นี่ สมาชิกต้องมีสิทธิ์ใช้งาน (license) ของผลิตภัณฑ์นั้น และตรวจสอบ URL ของ MCP ตามคู่มือของผลิตภัณฑ์", "Enter the Client ID and Client Secret here. Members need the relevant product licenses; also check the product's MCP endpoint in the guide."],
    ],
  },
  dropbox: {
    ...
    steps: [
      ["รายการเดิมนี้ใช้ปลายทางสำหรับ Claude ผู้ดูแล ORCA ต้องตรวจสอบวิธีย้ายไปยังปลายทาง MCP ปัจจุบันก่อนเปิดให้เชื่อมต่อใหม่", "This legacy entry uses a Claude-specific endpoint. ORCA's administrator must review migration to the current MCP endpoint before enabling new connections."],
      ["Dropbox เปิดให้ลงทะเบียนอัตโนมัติเฉพาะไคลเอนต์ที่เชื่อถือได้ ส่วนไคลเอนต์อื่นต้องตั้งค่าแอป Dropbox ตามคู่มือ", "Dropbox offers automatic registration only to trusted clients. Other clients need a Dropbox app configured using the guide."],
    ],
  },
  harvey: {
    ...
    steps: [
      ["ให้ผู้ดูแลบัญชีติดต่อ Harvey เพื่อเปิดใช้ MCP และยืนยันว่าอนุญาตให้ ORCA ลงทะเบียนเป็นไคลเอนต์ได้", "Ask your account administrator to contact Harvey to enable MCP and confirm that ORCA can register as a client."],
      ["ขอขั้นตอนลงทะเบียน ORCA และการตั้งค่า OAuth จาก Harvey ก่อน คู่มือสาธารณะยังไม่ระบุวิธีลงทะเบียนไคลเอนต์ที่กำหนดเอง", "Obtain ORCA registration and OAuth setup instructions from Harvey first. The public guide does not document custom-client registration."],
    ],
  },
...
function managedOAuthSetup(sourceID: string, provider?: string): OAuthProviderSetup | undefined {
  ...
  if (api) return {
    ...
    steps: [
      ['ใช้แอป OAuth ของ ORCA ในโครงการ Google Cloud และตั้งค่า Branding เป็น ORCA', 'Use the ORCA OAuth app in its Google Cloud project and set Branding to ORCA.'],
      [`เปิดใช้ ${api} และเพิ่มสิทธิ์ที่ตัวเชื่อมต้องใช้ในส่วน Data Access หากแอปยังอยู่ในสถานะ Testing ให้เพิ่มบัญชีทดสอบในส่วน Audience`, `Enable ${api} and add the connector scopes in Data Access. While the app is in Testing, add the test accounts in Audience.`],
  ...
  return {
    ...
    steps: [
      ...
      [`เพิ่มสิทธิ์ Microsoft Graph แบบ Delegated permission ได้แก่ ${scope} และขอการอนุมัติจากผู้ดูแลระบบหากนโยบายองค์กรกำหนด`, `Add the Microsoft Graph delegated permission ${scope}. Request administrator consent when the organization requires it.`],
      ...(provider === 'microsoft-excel' ? [[
        'Excel API ต้องใช้สิทธิ์ Files.ReadWrite แม้เครื่องมือ ORCA ชุดนี้จะอ่านข้อมูลเท่านั้น',

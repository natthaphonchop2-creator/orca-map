...
<ResponsiveDialog bind:this={dialog} hideClose disableClickOutside class="text-md w-sm">
	...
	<h2 class="mb-8 text-center text-2xl font-semibold">Welcome to ORCA!</h2>

	<div class="w-fit self-center">
		{#if !isAuthProviderConfigured || requiresModelProviderConfiguration}
			{#if isBootstrapUser}
				<p>Before using ORCA, you'll need to:</p>
			{:else}
				<p class="text-center">
		...
		<p class="pt-4">
			Continue after reviewing the ORCA
			<a href="/terms-of-service" target="_blank" rel="noopener" class="text-link">installation notice</a>
			and <a href="/third-party.html" target="_blank" rel="noopener" class="text-link">open-source licenses</a>.
		</p>
	</div>

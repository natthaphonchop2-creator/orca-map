<script lang="ts">
	...
	async function saveOrganization() {
		...
		success = '';
		try {
			const saved = await OrcaService.organization(displayName.trim(), organizationVersion);
	...
	async function reload() {
		...
		if (editingUnit) editUnit(data.units.find((unit) => unit.id === editingUnit?.id));
		error = '';
	}
</script>
...
<div class="k-breadcrumb">
	<a href={localeHref('/app?view=workspaces')}>{t('พื้นที่ทำงาน', 'Workspaces')}</a><span>/</span><span
		>{t('ตั้งค่าองค์กร', 'Organization settings')}</span
	>
</div>
<div class="k-intro">
...
{#if error}<div class="k-banner error" role="alert">
		...
		<div>
			{error}
			<div class="k-actions">
				<button class="k-link-button" disabled={orgBusy || unitBusy} onclick={reload}
...
<section class="k-panel">
	...
	>
		<fieldset disabled={!data.canManage || orgBusy}>
			<div class="k-grid-2">
				<div class="k-field">
					<label for="organization-name">{t('ชื่อองค์กร', 'Organization name')}</label><input
				...
				<div class="k-field">
					<label for="organization-timezone">{t('เขตเวลา', 'Time zone')}</label><input
			...
			{#if data.canManage}<button
					...
					type="submit"
					disabled={orgBusy || !displayName.trim()}
...
<section class="k-section">
	<div class="k-section-title">
		...
		{#if data.canManage}<button
				...
				disabled={unitBusy}
				onclick={() => editUnit()}><Plus size={16} /> {t('เพิ่มหน่วยงาน', 'Add unit')}</button
			>{/if}
	</div>
	...
	{#if unitForm && data.canManage}<form
		...
		>
			<h3>{editingUnit ? t('แก้ไขหน่วยงาน', 'Update unit') : t('เพิ่มหน่วยงาน', 'Add unit')}</h3>
			<fieldset disabled={unitBusy}>
				<div class="k-grid-2 k-section">
					<div class="k-field">
						<label for="unit-name">{t('ชื่อหน่วยงาน', 'Unit name')}</label><input
							id="unit-name"
							bind:value={unitName}
							required
							maxlength="100"
					...
					<div class="k-field">
						<label for="unit-kind">{t('ประเภท', 'Type')}</label><select
							id="unit-kind"
							bind:value={kind}
							>{#each Object.entries(unitLabels) as [value, label]}<option {value}>{label}</option
								>{/each}</select
						>
					</div>
				</div>
				<div class="k-field">
					<label for="unit-parent">{t('สังกัด', 'Parent unit')}</label><select
						id="unit-parent"
						bind:value={parentID}
						><option value="">{t('ขึ้นตรงกับองค์กร', 'Directly under the organization')}</option
				...
				<div class="k-actions" style="margin-top:19px">
					<button class="k-button primary" type="submit" disabled={unitBusy || !unitName.trim()}
						>{unitBusy ? t('กำลังบันทึก…', 'Saving…') : t('บันทึกหน่วยงาน', 'Save unit')}</button
					><button
						class="k-button"
						type="button"
						disabled={unitBusy}
	...
	{#if data.units.length}<div class="k-table-wrap">
			<table class="k-table">
				<thead
					><tr
						><th>{t('หน่วยงาน', 'Organization units')}</th><th>{t('ประเภท', 'Type')}</th><th
							>{t('สังกัด', 'Parent unit')}</th
						>{#if data.canManage}<th>{t('จัดการ', 'Manage')}</th>{/if}</tr
					></thead
				><tbody
					>{#each data.units as unit}<tr
							><td>{unit.name}</td><td>{unitLabels[unit.kind]}</td><td
								>{data.units.find((parent) => parent.id === unit.parentID)?.name ||
									data.organization.displayName}</td
							>{#if data.canManage}<td
									><button
										class="k-button quiet small"
										disabled={unitBusy}
										onclick={() => editUnit(unit)}><Pencil size={14} /> {t('แก้ไข', 'Edit')}</button
									></td
								>{/if}</tr
						>{/each}</tbody
				>

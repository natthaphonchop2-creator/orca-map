...
export function component(deps) { const { testAccounts, testStatus, testLocalAvailable, beforeNavigate, organizationRole, canResetMemberPassword, OrcaLibraryService, TeamLifecycleActions, LibraryDepartments, MemberRoleEditor, LOCAL_AUTH_MIN_PASSWORD_LENGTH, t,localeHref,OrcaService,orcaError,memberName,memberRole,Building2,Check,Copy,Crown,Info,KeyRound,Plus,RefreshCw,Shield,Users,onMount,onDestroy }=deps; ${code}; return TeamAccess; }`;
...
function screen(actorRole,targets=[],props={},testState={}) {
 ...
 const view=component({testAccounts:testState.accounts||[],testStatus:testState.status||'active',testLocalAvailable:testState.localAvailable||false,beforeNavigate:noop,organizationRole,canResetMemberPassword,OrcaLibraryService:{},TeamLifecycleActions:(_r,input)=>actions.push(input),LibraryDepartments:noop,MemberRoleEditor:noop,LOCAL_AUTH_MIN_PASSWORD_LENGTH:12,t:(_th,en)=>en,localeHref:x=>x,OrcaService:{},orcaError:()=>'',memberName:m=>m.displayName||m.email,memberRole:organizationRole,Building2:noop,Check:noop,Copy:noop,Crown:noop,Info:noop,KeyRound:noop,Plus:noop,RefreshCw:noop,Shield:noop,Users:noop,onMount:noop,onDestroy:noop});
